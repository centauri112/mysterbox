...
/**
 * Converts a chain id (listed at at chainlist.org) to a ZeroBridge domain.
 *
 * @param chainId A chain id number
 * @returns A ZeroBridge domain number in decimal
 */
export function chainIdToDomain(chainId: number): number {
  const domain = chainIdToDomainMapping.get(chainId);
  if (!domain)
    throw new Error(
      `Cannot find corresponding ZeroBridge domain for chainId ${chainId}`,
    );
...
/**
 * Converts a string (e.g. "eth" for Ethereum) to a ZeroBridge domain displayed as
 * a hex string.
 * @dev Interprets string bytes as int.
 * @param name The chain string
 * @returns A 0x prefixed ZeroBridge domain in hex (string)
 */
...
/**
 * Converts a string (e.g. "eth" for Ethereum) to a decimal formatted ZeroBridge
 * domain.
 * @dev Interprets string bytes as int.
 * @param name The chain string
 * @returns A ZeroBridge domain number in decimal
...
/**
 * Converts an ZeroBridge ID of 20 or 32 bytes to the corresponding EVM Address.
 *
 ...
 * @throws if either side is not a valid 20 or 32 byte zerobridge identifier
 */
...
export class UnreachableError extends Error {
  constructor(extra?: string) {
    super(
      `Unreachable. You should not see this Error. Please file an issue at https://github.com/zerobridge-xyz/monorepo, including the full error output. Extra info: ${
        extra ?? 'none'
      }`,
    );
  }
