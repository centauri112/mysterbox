...
import {UpgradeBeaconProxy} from "@zerobridge-xyz/contracts-core/contracts/upgrade/UpgradeBeaconProxy.sol";
import {UpgradeBeacon} from "@zerobridge-xyz/contracts-core/contracts/upgrade/UpgradeBeacon.sol";
import {UpgradeBeaconController} from "@zerobridge-xyz/contracts-core/contracts/upgrade/UpgradeBeaconController.sol";
import {UpdaterManager} from "@zerobridge-xyz/contracts-core/contracts/UpdaterManager.sol";
import {XAppConnectionManager} from "@zerobridge-xyz/contracts-core/contracts/XAppConnectionManager.sol";
import {Home} from "@zerobridge-xyz/contracts-core/contracts/Home.sol";
import {Replica} from "@zerobridge-xyz/contracts-core/contracts/Replica.sol";
import {GovernanceRouter} from "@zerobridge-xyz/contracts-core/contracts/governance/GovernanceRouter.sol";
import {BridgeRouter} from "@zerobridge-xyz/contracts-bridge/contracts/BridgeRouter.sol";
import {TokenRegistry} from "@zerobridge-xyz/contracts-bridge/contracts/TokenRegistry.sol";
import {ETHHelper} from "@zerobridge-xyz/contracts-bridge/contracts/ETHHelper.sol";
import {AllowListNFTRecoveryAccountant} from "@zerobridge-xyz/contracts-bridge/contracts/accountants/NFTAccountant.sol";
...
abstract contract Config is INomadProtocol {
    ...
    function getCoreDeployHeight(
        string memory domain
    ) public view onlyInitialized returns (uint256) {
        return abi.decode(loadCoreAttribute(domain, "deployHeight"), (uint256));
    }

    function governanceRouterUpgrade(
        string memory domain
    ) public view override onlyInitialized returns (Upgrade memory) {
        return
            abi.decode(
    ...
    function getGovernanceRouter(
        string memory domain
    ) public view override onlyInitialized returns (GovernanceRouter) {
    ...
    function getHome(
        string memory domain
    ) public view override onlyInitialized returns (Home) {
        return Home(address(homeUpgrade(domain).proxy));
    }

    function getHomeImpl(
        string memory domain
    ) public view onlyInitialized returns (Home) {
        return Home(address(homeUpgrade(domain).implementation));
    }

    function getUpdaterManager(
        string memory domain
    ) public view override onlyInitialized returns (UpdaterManager) {
        return
            abi.decode(
    ...
    function getUpgradeBeaconController(
        string memory domain
    ) public view override returns (UpgradeBeaconController) {
        return
            abi.decode(
    ...
    function getXAppConnectionManager(
        string memory domain
    ) public view override returns (XAppConnectionManager) {
        return
            abi.decode(
    ...
    function replicaOfUpgrade(
        string memory local,
        string memory remote
    ) public view override returns (Upgrade memory) {
        return
            abi.decode(
    ...
    function getReplicaOf(
        string memory local,
        string memory remote
    ) public view override returns (Replica) {
    ...
    function getBridgeDeployHeight(
        string memory domain
    ) public view onlyInitialized returns (uint256) {
        return
    ...
    function getBridgeRouter(
        string memory domain
    ) public view override onlyInitialized returns (BridgeRouter) {
        return BridgeRouter(address(bridgeRouterUpgrade(domain).proxy));
    }

    function bridgeTokenUpgrade(
        string memory domain
    ) public view override onlyInitialized returns (Upgrade memory) {
        return
    ...
    function getTokenRegistry(
        string memory domain
    ) public view override onlyInitialized returns (TokenRegistry) {
        return TokenRegistry(address(tokenRegistryUpgrade(domain).proxy));
    }

    function accountantUpgrade(
        string memory domain
    )
        public
        view
    ...
    function getEthHelper(
        string memory domain
    ) public view override returns (ETHHelper) {
    ...
    function protocolAttributePath(
        string memory domain,
        string memory key
    ) internal pure returns (string memory) {
    ...
    function loadProtocolAttribute(
        string memory domain,
        string memory key
    ...
    function getConnections(
        string memory domain
    ) public view override onlyInitialized returns (string[] memory) {
        return
            abi.decode(
    ...
    function getDomainNumber(
        string memory domain
    ) public view onlyInitialized returns (uint32) {
        return abi.decode(loadProtocolAttribute(domain, "domain"), (uint32));
    }

    function getUpdater(
        string memory domain
    ) public view override onlyInitialized returns (address) {
        return
            abi.decode(
    ...
    function getWatchers(
        string memory domain
    ) public view override onlyInitialized returns (address[] memory) {
        return
            abi.decode(
