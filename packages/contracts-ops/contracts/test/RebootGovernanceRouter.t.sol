// SPDX-License-Identifier: MIT OR Apache-2.0
pragma solidity 0.7.6;
pragma abicoder v2;

import {RebootTest} from "./Reboot.t.sol";
import {NomadTest} from "@zerobridge-xyz/contracts-core/contracts/test/utils/NomadTest.sol";
import {GoodXappSimple} from "@zerobridge-xyz/contracts-core/contracts/test/utils/GoodXapps.sol";
import {GovernanceRouterTest} from "@zerobridge-xyz/contracts-core/contracts/test/GovernanceRouter.t.sol";
import {GovernanceRouterHarness} from "@zerobridge-xyz/contracts-core/contracts/test/harnesses/GovernanceRouterHarness.sol";
import {TypeCasts} from "@zerobridge-xyz/contracts-core/contracts/XAppConnectionManager.sol";
import {MockHome} from "@zerobridge-xyz/contracts-bridge/contracts/test/utils/MockHome.sol";
import {MockXAppConnectionManager} from "@zerobridge-xyz/contracts-core/contracts/test/utils/MockXAppConnectionManager.sol";

contract GovernanceRouterRebootTest is RebootTest, GovernanceRouterTest {
    address govHarnessImpl;

    function setUp() public override(NomadTest, GovernanceRouterTest) {
        setUpReboot("governanceRouter");
        // upgrade to harness
        governanceRouter = GovernanceRouterHarness(
            address(getGovernanceRouter(localDomainName))
        );
        setUp_upgradeGovernanceRouterHarness();
        // set test vars
        recoveryManager = getRecoveryManager(localDomainName);
        remoteGovernanceRouter = TypeCasts.addressToBytes32(
            address(getGovernanceRouter(remote))
        );
        xAppConnectionManager = getXAppConnectionManager(localDomainName);
        home = MockHome(address(getHome(localDomainName)));
        // set up fixtures
        GovernanceRouterTest.setUp_testFixtures();
    }

    function setUp_upgradeGovernanceRouterHarness() public {
        // GOV ROUTER
        govHarnessImpl = address(
            new GovernanceRouterHarness(
                getDomainNumber(localDomainName),
                getRecoveryTimelock(localDomainName)
            )
        );
        vm.writeJson(
            vm.toString(govHarnessImpl),
            outputPath,
            coreAttributePath(
                localDomainName,
                "governanceRouter.implementation"
            )
        );
        reloadConfig();
        pushSingleUpgrade(
            governanceRouterUpgrade(localDomainName),
            localDomainName
        );
        prankExecuteRecoveryManager(
            address(getGovernanceRouter(localDomainName)),
            getDomainNumber(localDomainName)
        );
    }

    // check fork setUp
    function test_setUp() public view {
        governanceRouter.exposed_isGovernorRouter(0, bytes32(0));
    }
}
