...
import { MultiProvider } from '@zerobridge-xyz/multi-provider';
import * as core from '@zerobridge-xyz/contracts-core';
...
/**
 * The NomadContext manages connections to ZeroBridge core and Bridge contracts.
...
export class NomadContext extends MultiProvider<config.Domain> {
  ...
  /**
   * Discovers the governor domain of this zerobridge deployment and returns the
   * associated Core.
   *
  ...
  /**
   * Fetch a config from the ZeroBridge config static site.
   *
  ...
  static async fetchConfig(environment: string): Promise<config.NomadConfig> {
    const uri = `https://zerobridge-xyz.github.io/config/${environment}.json`;
  ...
  /**
   * Fetch a config from the ZeroBridge config static site and instantiate a context
