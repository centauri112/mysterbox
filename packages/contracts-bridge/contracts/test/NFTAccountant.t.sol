...
contract NFTAccountantTest is AccountantTest {
    ...
    function test_initValues() public {
        ...
        assertEq(accountant.name(), "ZeroBridge NFT");
        assertEq(accountant.symbol(), "noNFT");
        assertEq(accountant.baseURI(), "https://nft.zerobridge.xyz/");
    }
    ...
    function checkUserAndGetAsset(
        address _user,
        uint8 _assetIndex
    ) internal returns (address payable _asset) {
        vm.assume(canAcceptNft(_user));
