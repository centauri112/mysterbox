...
// ============ External Contracts ============
import {XAppConnectionClient} from "@zerobridge-xyz/contracts-router/contracts/XAppConnectionClient.sol";
...
contract TokenRegistry is Initializable, XAppConnectionClient, ITokenRegistry {
     ...
     */
    function getRepresentationAddress(
        uint32 _domain,
        bytes32 _id
    ) public view returns (address _representation) {
    ...
    function ensureLocalToken(
        uint32 _domain,
        bytes32 _id
    ) external override onlyOwner returns (address _local) {
     ...
     */
    function oldReprToCurrentRepr(
        address _oldRepr
    ) external view override returns (address _currentRepr) {
    ...
    function getTokenId(
        address _local
    ) external view override returns (uint32 _domain, bytes32 _id) {
        BridgeMessage.TokenId memory _tokenId = representationToCanonical[
            _local
        ];
    ...
    function getLocalAddress(
        uint32 _domain,
        address _id
    ) external view returns (address _local) {
    ...
    function getLocalAddress(
        uint32 _domain,
        bytes32 _id
    ) public view override returns (address _local) {
