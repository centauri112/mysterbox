...
export class HardhatNetwork extends Network {
  ...
  constructor(
    ...
    wETHAddress?: string,
    docker?: Dockerode,
    options?: HardhatNetworkOptions,
    blockExplorer?: string
  ) {
    super(name, domain, options?.chainId || domain);
    this.docker = docker || new Dockerode();
    this.handler =
      options?.handler || new DockerizedNetworkActor(this.name, this.docker);
    this.blockTime = 10;
  ...
  /* TODO: reimplement abstractions for MULTIPLE hardhat networks (i.e. any ZeroBridge domain).
