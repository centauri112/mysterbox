 ...
 */
  ...
  async createContainer(): Promise<Dockerode.Container> {
    ...
    const connectionUrls =
      this.nomadEnv?.domains.map(
        (d) => `${d.domain.name.toUpperCase()}_CONNECTION_URL=${d.rpcs[0]}`
      ) || [];

    // docker run --name $1_$2_agent --env RUN_ENV=main --restart=always --network="host" --env BASE_CONFIG=$1_config.json -v $(pwd)/../../rust/config:/app/config -d gcr.io/zerobridge-xyz/zerobridge-agent ./$2
