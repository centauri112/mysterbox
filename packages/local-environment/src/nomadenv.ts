import { NomadLocator, NomadConfig } from "@zerobridge-xyz/configuration";
import { DeployContext } from "@zerobridge-xyz/deploy/src/DeployContext";
import * as ethers from "ethers";
import { NonceManager } from "@ethersproject/experimental";
import fs from "fs";
import bunyan from "bunyan";
import { NomadDomain } from "./domain";
import { BridgeContext } from "@zerobridge-xyz/sdk-bridge";
import { NomadContext } from "@zerobridge-xyz/sdk";
...
export async function defaultStart(): Promise<NomadEnv> {
  ...
  const tom = NomadDomain.newHardhatNetwork("tom", tDomainNumber, {
    forkurl: `${process.env.ALCHEMY_FORK_URL}`,
  ...
  for (const domain of le.domains) {
    if (!domain.network.weth) {
      await Promise.all([
        domain.network.setWETH(await domain.network.deployWETH()),
      ]);
    }
  }
