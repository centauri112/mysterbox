...
export class DockerizedBinary {
   ...
   * @param {string} dockerImage - e.g., `gcr.io/zerobridge-xyz/zerobridge-agent:some-tag`
   * @param {Docker.ContainerCreateOptions} containerOptions - see `Docker.createContainer(..)`
   */
  public constructor(
    dockerImage: string,
    containerOptions: Docker.ContainerCreateOptions
  ) {
  ...
  public async run(cmd: string[]): Promise<string> {
    return this.docker
      .run(this.dockerImage, cmd, this.outputStream, this.containerOptions)
      .then((result) => {
