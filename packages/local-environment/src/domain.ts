import {
  ...
  ProcessorConfig,
  Domain,
} from "@zerobridge-xyz/configuration";
...
export class NomadDomain {
  ...
  static newHardhatNetwork(
    name: string,
    domain: number,
    creationParams?: DomainOptions
  ): NomadDomain {
    const network = new HardhatNetwork(
      name,
      domain,
      creationParams?.forkurl,
      creationParams?.weth,
      creationParams?.docker
    );
  ...
  ensureAgents(metricsPort = 9090, docker?: Dockerode): void {
    if (!this.agents)
      this.agents = new Agents(this, metricsPort, docker, this.nomadEnv);
  }

  connectDomain(d: NomadDomain, connectInReturn = true): void {
    if (
      !this.connections().includes(d.network.name) &&
      d.name != this.network.name
    ) {
  ...
  */
  ...
  async down(): Promise<[void | undefined, void]> {
    return await Promise.all([this.downAgents(), this.downNetwork()]);
  }
  ...
  processorConfig(subsidizedRemotes?: string[]): ProcessorConfig {
    if (subsidizedRemotes || this.nomadEnv) {
      subsidizedRemotes =
        subsidizedRemotes || this.nomadEnv?.getDomains().map((d) => d.name);
    } else {
