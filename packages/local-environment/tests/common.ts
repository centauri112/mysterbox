// import { ZeroBridge, utils, Network, LocalNetwork, Key } from "../src";
import type { TokenIdentifier } from "@zerobridge-xyz/sdk-bridge/src";
import { ethers } from "ethers";
import { BridgeToken } from "@zerobridge-xyz/contracts-bridge/";
// import { TransferMessage } from "@zerobridge-xyz/sdk/zerobridge";
...
import { TransferMessage } from "@zerobridge-xyz/sdk-bridge";
 ...
 *
 * @param n - ZeroBridge instance which has both "from" and "to" networks
 ...
 * @param token - token identifier according to ZeroBridge
...
export async function sendTokensAndConfirm(
  ...
  for (const a of amounts) {
    ...
    log.info(
      `Going to send token ${token.domain}:${token.id}`,
      `from ${from.name}`,
      `to ${to.name}`
    );
    ...
    const replica = ctx
      .mustGetCore(to.domain.domain)
      .getReplica(from.domain.domain);
    if (replica) {
      log.info(`Waiting for update and process events...`);
      await new Promise((resolve, reject) => {
        replica.once(
          replica.filters.Update(null, null, null, null),
          (homeDomain, oldRoot, newRoot, _signature) => {
            log.info(
              `New Update event | homeDomain: ${homeDomain} | oldRoot: ${oldRoot} | newRoot: ${newRoot}`
            );
          }
        );

        replica.once(
          replica.filters.Process(null, null, null),
          (messageHash, success, _returnData) => {
            log.info(
              `New Process event | messageHash: ${messageHash} | success:`,
              success
            );
            resolve(null);
          }
  ...
  log.info(
    `Waiting for all assets to be delivered at from: ${from.name}, to: ${to.name} . Batch:${batch}!`
  );
  ...
  if (tokenContract === null)
    throw new Error(`Timedout token creation at destination`);
  ...
  log.info(
    `Recipient's balance on recipient (${recipient}) domain ${to.name} is`,
    (await tokenContract.balanceOf(recipient)).toString()
  );
...
export async function setupTwo(log: Logger): Promise<{ le: NomadEnv }> {
  // Instantiate ZeroBridge domains
  ...
  const tom = NomadDomain.newHardhatNetwork("tom", tDomainNumber, {
    forkurl: le.forkUrl,
    weth: le.wETHAddress,
    nomadEnv: le,
  });
  const jerry = NomadDomain.newHardhatNetwork("jerry", jDomainNumber, {
    forkurl: le.forkUrl,
    weth: le.wETHAddress,
    nomadEnv: le,
  });
  ...
  const [tweth, jweth] = await Promise.all([
    le.tDomain?.network.deployWETH(),
    le.jDomain?.network.deployWETH(),
  ]);
...
export async function setupTwo() {
  ...
  fs.writeFileSync("/tmp/zerobridge.json", JSON.stringify(n.toObject()));
