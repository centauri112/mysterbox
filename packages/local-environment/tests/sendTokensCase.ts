...
import { ParsedMessage } from "@zerobridge-xyz/sdk";
...
(async () => {
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        `Resolved asset at destination Jerry is not the same as the token. ${tokenContract.address.toLowerCase()} != ${token.id
          .toString()
          .toLowerCase()}`
      );
    } else {
