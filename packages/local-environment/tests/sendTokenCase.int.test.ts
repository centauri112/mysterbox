...
import type { TokenIdentifier } from "@zerobridge-xyz/sdk-bridge";
import fs from "fs";
...
describe("Token test", () => {
  ...
  if (process.env.ALCHEMY_FORK_URL) {
    log.info(
      `Using Alchemy API ` +
        process.env.ALCHEMY_FORK_URL +
        ` to start a forked network`
    );
  }
  ...
  async function setUp() {
    ...
    const [tweth, jweth] = await Promise.all([
      tDomain.network.deployWETH(),
      jDomain.network.deployWETH(),
    ]);
  ...
  it("should handle token creation, transfer logic", async function () {
    ...
    expect(tokenContract.address.toLowerCase()).equal(
