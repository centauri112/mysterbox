...
contract DABridgeRouter is Version0, Router {
    ...
    /**
     * @notice Receive messages sent via ZeroBridge from other remote xApp Routers;
     * parse the contents of the message and enact the message's effects on the local chain
     * @dev Called by an ZeroBridge Replica contract while processing a message sent via ZeroBridge
    ...
    function _parse(
        bytes29 _message
    ) internal pure returns (uint32 blockNumber, bytes32 dataRoot) {
