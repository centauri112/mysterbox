...
contract GovernanceRouterTest is NomadTest {
    ...
    function test_callRemoteOnlyGovernorFuzzed(
        address nonGovernor
    ) public runNotInRecovery {
        vm.assume(
    ...
    function test_transferGovernorLocalGovernorFuzzed(
        address newGovernor
    ) public runNotInRecovery {
        uint32 newDomain = homeDomain;
        if (newGovernor == address(0)) {
    ...
    function test_transferRecoveryManagerFuzzed(
        address newRecoveryManager
    ) public {
        vm.startPrank(recoveryManager);
        vm.expectEmit(true, true, false, false);
    ...
    function test_handleBatchSuccessFuzzed(
        address to,
        bytes memory data
    ) public {
        calls.push(GovernanceMessage.Call(to.addressToBytes32(), data));
